...
export const fetchItems =
  (sortBy, gender, page, categoryId, size, price, brand) =>
  async (dispatch) => {
...
export const getCategories = (gender) => (dispatch) => {
  axios
    .get(`http://localhost:5000/api/categories/${gender}`)
    .then(({ data }) => {
...
export const getCategory = (gender, id) => (dispatch) => {
  ...
  axios
    .get(`http://localhost:5000/api/categories/${gender}${idQuery}`)
    .then(({ data }) => {
...
export const fetchItemsBySearch = (searchQuery, gender) => async (dispatch) => {
  await axios
    .get(
      `http://localhost:5000/api/items/search?q=${
        searchQuery || 'none'
      }&gender=${gender}`
    )
    .then(({ data }) => {
